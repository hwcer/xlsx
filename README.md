...
| 单元格   | 规则                                                                                      |
|----------|-------------------------------------------------------------------------------------------|
| A1       | 数据表名(必填),支持 `S:`/`C:` 前缀,经 `TrimProtoName` 归一化为 proto message 名       |
...
| `<dummy>field{...}`       | 等价于 `field.dummy{...}`,显式指定子结构体名            |
| `<dummy>field[{...}]`     | 等价于 `field.dummy[{...}]`,显式指定子结构体名          |
| `field.dummy{...}`        | 对象字段,显式指定子结构体名为 `dummy`                   |
| `field.dummy[{...}]`      | 数组对象字段,显式指定子结构体名为 `dummy`               |
| `name#branch`             | 版本分支字段,不同 branch 共享同一 proto 字段位置        |

显式指定子结构体名时,`.` 之后的部分(或 `<>` 内的部分)会经 `TrimProtoName` 格式化为 PROTO 命名规范(首字母大写、剥离下划线驼峰化)。若未显式指定,子结构体名会根据字段签名自动生成,可配合 `Config.EnableGlobalDummyName` 控制是否保留自定义名。

#### 前后端字段标记 (`S:` / `C:`)

字段名和表名支持 `S:` / `C:` 前缀,用于区分前后端归属:

| 写法         | 含义                                           |
|--------------|------------------------------------------------|
| `val`        | 无前缀,前后端公用                             |
| `S:val`      | 仅服务器使用,`--tag=S` 时导出                 |
| `C:val`      | 仅客户端使用,`--tag=C` 时导出                 |

- 前缀由 `TrimProtoName` 解析,返回 `(tag, name)`,`tag` 存入 `Field.side` / `Sheet.side`(不可变),`name` 为格式化后的字段名
- 通过 `VerifyTag(tag)` 判断是否匹配当前 `--tag` 配置,不匹配的字段/表整体跳过
- 前缀对表名同样生效:A1 填 `S:ItemConfig` 表示该表仅在 `--tag=S` 时导出
